...
- Before asking a signed-out user to log in, resolve and probe the Tailscale coordination server. Report proxy virtual addresses in `198.18.0.0/15` separately from ordinary network failures, so users get a concrete VPN or real-DNS instruction instead of a login command that waits without showing a URL.
...
- Core 1.0.8 has no public authentication-event interface. Start it with debug-level JSON request logging and parse only lines containing the required `ts`, `level`, and `event` fields. Ignore ordinary text and health checks. A rejected `/mcp` request or a browser request from `chatgpt.com` means authorization is needed; `mcp_session_created` is the only event that means an authorized MCP session currently exists. Keep a stored OAuth authorization in the separate neutral `configured` state instead of presenting it as a live connection.
- Open the official ChatGPT custom-app page at `https://chatgpt.com/plugins`. The connection management window remains available in every ChatGPT state and always provides explicit copy actions for the MCP address and Owner Password. Starting a reconnect copies the Owner Password before opening ChatGPT, waits for a real MCP session for 90 seconds, and then shows a concrete VPN and secure-connection check if no session arrives. Availability can still depend on the user's ChatGPT account and workspace policy.
